...
https://mission-driven-frontend-assignment-iota.vercel.app/
...
- **이미지 삭제**: 업로드된 메인 이미지 및 추가 이미지 삭제 기능
...
- **제출 확인 모달**: 모든 필수 항목 완료 시 "등록하시겠습니까?" 확인 모달 표시
- **로딩 스피너**: 등록 버튼 클릭 시 2초간 로딩 스피너 표시
- **완료 알림**: 등록 완료 후 토스트 메시지 표시
...
- 이미지 삭제 (삭제 버튼 클릭, ImageProvider 상태 업데이트)
- 상호작용 (이미지 클릭, 재선택)
...
- 이미지 삭제 (개별 이미지 삭제, ImageProvider 상태 업데이트)
- 4장 제한 (최대 개수 초과 시 처리)
- ImageProvider 연동 (상태 동기화)
- 접근성
...
- 클릭 동작 (라우팅, 조건에 따른 동작)
- 최종 제출 모달 (모든 항목 완료 시 모달 표시, 등록 확인)
- 로딩 상태 관리 (로딩 스피너 표시, 2초 타이머)
- 접근성
